...
export class EredmenyComponent implements OnInit {
  ...
  loadResults(): void {
    forkJoin({
      votings: this.http.get<VotingResult[]>('https://hecarfest-backend2.onrender.com/api/votings').pipe(
        catchError((error: HttpErrorResponse) => {
          console.error('Hiba történt a szavazatok lekérésekor:', error);
          return of([]);
        })
      ),
      votes: this.http.get<VoteDetails[]>('https://hecarfest-backend2.onrender.com/api/voting/details').pipe(
      ...
      next: ({ votings, votes }) => {
        this.votings = votings;
        this.votes = votes;
        console.log('Szavazatok:', this.votes);
        this.loadCategories();
      },
      error: (err) => {
        console.error('Hiba az adatok lekérésénél:', err);
    ...
    this.http.get<{ data: Category[] }>('https://hecarfest-backend2.onrender.com/api/category')
      ...
      .subscribe({
        next: (response) => {
          ...
          console.log('Lekért kategóriák:', this.categories);
          this.mapResults();
        },
        error: (err) => {
          console.error('Hiba a kategóriák lekérésekor:', err);
  ...
  mapResults(): void {
    this.results = this.categories.map(category => {
      const votesInCategory = this.votes.filter(vote => vote.category === category._id);
      console.log(`Kategória: ${category.name}, Szavazatok:`, votesInCategory);
      return {
        category: category,
        votes: votesInCategory,
        voteDetails: votesInCategory,
        carVotes: this.calculateCarVotes(category._id),
        count: votesInCategory.reduce((sum, vote) => sum + (this.votings.find(v => v._id === vote._id)?.count || 0), 0) 
      };
    });
    console.log('Összesített eredmények:', this.results);
  }
  ...
  private calculateCarVotes(categoryId: string): CarVotes[] {
    ...
    const carVotesArray = Array.from(carVotesMap.entries())
      .map(([licence_plate, data]) => ({
        licence_plate,
        vote_count: data.count,
        last_vote_date: data.lastVoteDate
      }))
      .sort((a, b) => b.vote_count - a.vote_count || (new Date(b.last_vote_date!).getTime() - new Date(a.last_vote_date!).getTime()));
    console.log(`Autók szavazatai (${categoryId} kategória):`, carVotesArray);
    return carVotesArray;
  }
}
